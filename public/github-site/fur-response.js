...
export class FurResponseField {
  ...
  update(dt,skinPoints=null,stroke=[1,0,0],palmNormal=[0,1,0]) {
    ...
    if(!skinPoints&&this.activity<1e-5)return false;
    const release=Math.exp(-Math.min(dt,.05)/.28),follow=1-Math.exp(-Math.min(dt,.05)/.045);
    const bounds=[Infinity,Infinity,Infinity,-Infinity,-Infinity,-Infinity];
    if(skinPoints)for(let j=0;j<skinPoints.length;j+=3)for(let k=0;k<3;k++){
      bounds[k]=Math.min(bounds[k],skinPoints[j+k]-.55);bounds[k+3]=Math.max(bounds[k+3],skinPoints[j+k]+.55);
    }
    let activity=0;
    for(let i=0;i<this.count;i++){
      ...
      let height=.8,influence=0;
      const cosine=nx*palmNormal[0]+ny*palmNormal[1]+nz*palmNormal[2];
      if(skinPoints&&cosine>.18&&x>bounds[0]&&x<bounds[3]&&y>bounds[1]&&y<bounds[4]&&z>bounds[2]&&z<bounds[5]){
        for(let j=0;j<skinPoints.length;j+=3){
          ...
          if(effective<height){height=effective;influence=w;}
        }
      }
      let tx=0,ty=0,tz=0;
      if(influence>0){
      ...
      const cx=influence>0?palmNormal[0]:nx,cy=influence>0?palmNormal[1]:ny,cz=influence>0?palmNormal[2]:nz;
