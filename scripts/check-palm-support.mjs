...
const target=new THREE.Vector3(),outward=new THREE.Vector3(),center=new THREE.Vector3(),facing=new THREE.Vector3(),along=new THREE.Vector3();
...
const rows=[];
for(let step=0;step<=Math.round(STROKE_DURATION*120);step++) {
  const t=step/120,s=sampler(t);
  articulateHand(model,'pet',s.pressure,s.progress,s.liftPose,1/120,step===0);
  target.fromArray(s.palm).add(bodyCenter);outward.fromArray(s.orientation);supportNormal.fromArray(s.normal);
  placePalm(hand,target,outward,center,facing,along.fromArray(s.fingerDirection));
